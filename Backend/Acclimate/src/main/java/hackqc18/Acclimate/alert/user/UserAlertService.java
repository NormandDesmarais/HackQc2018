...
/**
 * Service class for User Alerts.
 * This is the class that implements all the dirty works to answer
 * requests from the controller and connect them with the model (Alert).
 */
...
public class UserAlertService {
    ...
    /**
     * Returns the list of alerts bounded by the north, south, east
    ...
    /**
     * Returns the alert for the provided id, null if no alert with
     * this id was found.
    ...
    /**
     * Construct a user alert from the AlertStub provided. If the alert
     * already exists, it increases the "count" number of this alert
     * and possibly change is "certitude" status accordingly.
     * @param stub an object containing the alert type, longitude and
    ...
    public Alert addAlert(AlertStub stub) {
        ...
        if (alert == null) {
            alert = new Alert(alertId, stub.getType()
                        ,stub.getLng(), stub.getLat());
        } else {
    ...
    /**
     * Increases the count of the alert referred by the provided id
     * and adjust the "certitude" status accordingly. It returns null
     * if no alert with this id was found.
    ...
    /**
     * Deletes the user alert with the providing id from the database.
     * Does nothing if there is no alert with this id.
     * @param id the alert id
     * @return the deleted alert or null if it doesn't exist
     */
    public Alert removeAlert(String id) {
        Alert alert = getAlert(id);
        if (alert != null) userAlertRepository.deleteById(id);
        return alert;
    }
    ...
    /**
     * Utility method that creates a unique id from the alert type,
     * longitude and latitude. Alerts of the same type located within
     * a 1 KM square will share the same id and will thus be identified
     * has the same alert.
     * @param stub
     * @return
     */
    ...
    /**
     * Utility method that computes the distance in KM of a longitude
     * or latitude coordinate from the zero value of this coordinate.
    ...
    /**
     * Utility method that increases the count of an alert and changes
     * its "certitude" status accordingly. It also updates the
     * "dateDeMiseAJour" property to the current date and time.
    ...
    /**
     * Utility method that compares the provided date with the current
     * date and returns true if the difference is greater than the expired
     * alert time delay.
     * @param date the date in the format AAAA-MM-JJTHH:MM:SS
     * @return true if the difference between the provided date and now
     * is greater than the alert expiration delay
     */
